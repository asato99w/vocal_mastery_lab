...
/// UVR-MDX-NET Voc_FT ボーカル分離 (Python実装と完全互換)
/// Voc_FT専用モデル: n_fft=7680, dim_f=3072
@available(iOS 17.0, macOS 14.0, *)
class MDXNetSeparatorVocFT {

    // MARK: - Constants (Voc_FTモデル専用)
    ...
    init(modelURL: URL) throws {
        print("🔄 MDXNetSeparatorVocFT 初期化中...")
        ...
        print("✅ MDXNetSeparatorVocFT 初期化完了")
    ...
    func separate(audioURL: URL, denoise: Bool = true) throws -> (vocals: [Float], instrumental: [Float]) {
        ...
        // 3. Voc_FTはボーカルモデル → モデル出力 = ボーカル
        //    伴奏 = 元音声 - ボーカル
        let vocals = sources
        var instrumental = [Float](repeating: 0, count: audio.left.count)

        // モノラル化して返す (左チャンネルのみ)
        for i in 0..<min(audio.left.count, sources.count) {
            instrumental[i] = audio.left[i] - sources[i]
        }
    ...
    /// STFT: PyTorchと同じ処理 (Voc_FT用: dim_f=3072)
    private func stft(left: [Float], right: [Float]) -> MLMultiArray {
        // 入力形状: [batch, channels, freq, time] = [1, 4, 3072, 256]
    ...
    private func predict(_ input: MLMultiArray) throws -> MLMultiArray {
        ...
        guard let result = output.featureValue(for: "var_992")?.multiArrayValue else {
            throw NSError(domain: "MDXNet", code: 4, userInfo: [NSLocalizedDescriptionKey: "Output 'var_992' not found"])
        }

        return result
    }

    /// iSTFT: PyTorchと同じ処理 (Voc_FT用: dim_f=3072)
