...
"""
周波数ビン問題の修正版
問題: STFT全体（2049）で処理しているが、モデルは2048しか扱わない
解決: UVRと同じく最初からdim_f=2048でSTFTを切り取り
"""
import numpy as np
import torch
...
print(f"   STFT shape (full): {stft_result.shape}")

# ✅ UVRと同じく dim_f で切り取り
stft_result = stft_result[..., :dim_f, :]
print(f"   STFT shape (dim_f切り取り後): {stft_result.shape}")

# ONNXモデル
session = ort.InferenceSession(model_path)

# 全チャンク処理
freq_bins = 2048  # = dim_f
time_frames = 256

total_freq = stft_result.shape[2]  # = dim_f = 2048
total_time = stft_result.shape[3]

print(f"\n🧠 全チャンク処理開始")
print(f"   周波数ビン: {total_freq} (= dim_f)")
print(f"   時間フレーム: {total_time}")
print(f"   チャンクサイズ: {freq_bins} x {time_frames}")

# ✅ 出力配列もdim_fサイズで作成
output_spectrogram = np.zeros((1, 4, dim_f, total_time), dtype=np.float32)
...
print(f"   出力スペクトログラムshape: {output_tensor.shape}")
print(f"   n (full frequency bins): {n}")

# ✅ UVRと同じく n - f でパディング
f_pad = torch.zeros([*batch_dims, c, n - f, t])
print(f"   周波数パディングshape: {f_pad.shape}")
...
if ratio < 0.5:
    print(f"   ⚠️  まだ不足: {1/ratio:.1f}x")
else:
    print(f"   ✅ 正常範囲")

# 保存
print(f"\n💾 結果保存:")
sf.write("tests/python_output/fixed_vocal.wav", vocal_numpy.T, sr)
sf.write("tests/python_output/fixed_instrumental.wav", instrumental_trimmed.T, sr)
print(f"   ボーカル: fixed_vocal.wav")
print(f"   インストゥルメンタル: fixed_instrumental.wav")
