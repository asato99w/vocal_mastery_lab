...
/// xcodebuild test -only-testing:VocalMasteringLabUITests/PlaybackUITests
/// ```
final class PlaybackUITests: XCTestCase {
    ...
    /// 再生中にピッチ検出が動作することを確認
    ///
    /// **重要**: ピッチ表示は `isActive && detectedPitch != nil` の条件で表示される。
    /// 再生終了後は `isPlayingRecording = false` となり、ピッチ表示が消える。
    /// そのため、このテストは再生中（再生終了前）にピッチ検出を確認する必要がある。
    ///
    /// 詳細: `claudedocs/active/playback-pitch-detection-test-fix.md`
    func testPlayback_shouldDetectPitch() throws {
        ...
        playButton.tap()
        let stopPlaybackButton = app.buttons["StopPlaybackButton"]
        XCTAssertTrue(stopPlaybackButton.waitForExistence(timeout: 5), "Stop playback button should appear during playback")

        // Screenshot: 再生開始直後
        let screenshot1 = app.screenshot()
        let attachment1 = XCTAttachment(screenshot: screenshot1)
        attachment1.name = "playback_pitch_01_playback_started"
        attachment1.lifetime = .keepAlways
        add(attachment1)

        // 3. ピッチ検出を確認（再生終了前に確認する）
        let detectedPitchNoteName = app.staticTexts["DetectedPitchNoteName"]
        XCTAssertTrue(detectedPitchNoteName.waitForExistence(timeout: 4), "Pitch should be detected during playback")

        // Screenshot: ピッチ検出成功
        let screenshot2 = app.screenshot()
        let attachment2 = XCTAttachment(screenshot: screenshot2)
        attachment2.name = "playback_pitch_02_detection_success"
        attachment2.lifetime = .keepAlways
        add(attachment2)
