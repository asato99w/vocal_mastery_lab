...
public struct RecordingStatistics: Equatable {
    ...
    public struct PositionStatistics: Equatable, Identifiable {
        ...
        /// Calculated from stable regions only (excludes legato transitions)
        public let averageDeviationCents: Double

        /// Standard deviation of pitch deviation
        public let deviationStdDev: Double

        /// Number of samples for this position (from stable regions)
        public let sampleCount: Int

        // MARK: - Note Detection Metrics (new)

        /// Note detection rate (0.0-1.0) - ratio of detected notes to total occurrences
        public let noteDetectionRate: Double

        /// Total occurrences of this position in the scale
        public let noteOccurrences: Int

        /// Number of notes successfully detected at this position
        public let notesDetected: Int

        // MARK: - Timing Metrics (new)

        /// Average onset error in milliseconds (signed: + = late, - = early)
        public let averageOnsetErrorMs: Double

        /// Onset accuracy (0.0-1.0) - ratio of onsets within tolerance
        public let onsetAccuracy: Double
        ...
        public init(
            ...
            deviationStdDev: Double,
            sampleCount: Int,
            noteDetectionRate: Double = 0,
            noteOccurrences: Int = 0,
            notesDetected: Int = 0,
            averageOnsetErrorMs: Double = 0,
            onsetAccuracy: Double = 0
        ) {
            self.position = position
            self.averageDeviationCents = averageDeviationCents
            self.deviationStdDev = deviationStdDev
            self.sampleCount = sampleCount
            self.noteDetectionRate = noteDetectionRate
            self.noteOccurrences = noteOccurrences
            self.notesDetected = notesDetected
            self.averageOnsetErrorMs = averageOnsetErrorMs
            self.onsetAccuracy = onsetAccuracy
        }
    }
